...
def tic_tac_toe(board):
    ...
    for col in range(len(board[0])):
        ...
        if output.count("X") == len(board):
            return "X"
        elif output.count("O") == len(board):
            return "O"
    ...
    for i in range(len(board)):
        output2 += board[i][i]
        if output2 == "X" * len(board):
            return "X"
        elif output2 == "O" * len(board):
            return "O"

    output3 = ""
    for i in range(len(board)):
            output3 += board[i][len(board)- 1 - i]
            if output3 == "X" * len(board):
                return "X"
            elif output3 == "O" * len(board):
                return "O"
