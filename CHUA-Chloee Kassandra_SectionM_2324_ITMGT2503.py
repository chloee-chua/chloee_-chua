...
with open('script.json') as script:
...
# To process the lines in the script, which is already imported and stored in jsondata variable , you need to do the following in chronological order:
#### Convert jsondata to string before all characters to uppercase characters
...
# replace punctuation with " " instead of ""

jsondata_string = str(jsondata)
jsondata_string = jsondata_string.upper()
...
for text in texts:
    jsondata_string = jsondata_string.replace(text, " ")

for punctuation in punctuation_list:
    jsondata_string = jsondata_string.replace(punctuation, " ")
...
for word in jsondata:
    if len(word) > 3:
        count = len(word)
        keys1 = jsondata.get(word)
        values1 = jsondata.get(count)
        wordcount_dictionary.append(keys1, values1)

wordcount_dictionary.sort(key=lambda x: x[1], reverse=True)
...
if (1000 * F) + (600 * R) + (750 * C) <= 18000 and (500 * F) + (750 * R) + (300 * C) <= 12000 and (50 * F) + (45 * R) + (75 * C) <= 1500 and (F >= R + 1 and F <= 3 * C):
