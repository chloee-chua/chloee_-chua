...
def bin_to_dec(binary_string):
    ...
    for i in range(len_string):
        bit_position = len_string - 1 - i 
        result += int(binary_string[i]) * (2**bit_position)
    return result
...
def telephone_cipher(message):
    ...
    for i in range(len(message)):
        current = message[i]
        current_value = encoder_dict[current]
        result += current_value 
        
        if i < len(message) - 1:
            next_value = encoder_dict[message[i + 1]]
            if current_value[0] == next_value[0]:
                result += "_"
...
def telephone_decipher(telephone_string):
    ...
    result = ""
    output = ""
    length_string = len(telephone_string)

    while counter < length_string:
        ...
        while counter < length_string - 1 and telephone_string[counter + 1] == character:
            output += character
            counter += 1
        
        output += character
        
        if output in decipher_dict:
            result += decipher_dict[output]
            output = ""
